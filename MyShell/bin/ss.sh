...
#	Description: BBR+BBR 魔改版 +Lotserver
#	Version: 1.1.7
#=================================================
...
installbbr(){
	...
	detele_kernel
	BBR_grub
	echo -e "${Tip} 重启 VPS 后，请重新运行脚本开启 ${Red_font_prefix}BBR/BBR 魔改版 ${Font_color_suffix}"
	stty erase '^H' && read -p "需要重启 VPS 后，才能开启 BBR/BBR 魔改版，是否现在重启 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重启中..."
		reboot
...
installlot(){
	...
	detele_kernel
	BBR_grub
	echo -e "${Tip} 重启 VPS 后，请重新运行脚本开启 ${Red_font_prefix}Lotserver${Font_color_suffix}"
	stty erase '^H' && read -p "需要重启 VPS 后，才能开启 Lotserver，是否现在重启 ? [Y/n] :" yn
	[ -z "${yn}" ] && yn="y"
	if [[ $yn == [Yy] ]]; then
		echo -e "${Info} VPS 重启中..."
		reboot
...
start_menu(){
clear
echo && echo -e " TCP 加速 一键安装管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  -- 就是爱生活 | 94ish.me --

 ${Green_font_prefix}0.${Font_color_suffix} 升级脚本
————————————内核管理————————————
 ${Green_font_prefix}1.${Font_color_suffix} 安装 BBR/BBR 魔改版内核
 ${Green_font_prefix}2.${Font_color_suffix} 安装 Lotserver（锐速）内核
————————————加速管理————————————
 ${Green_font_prefix}3.${Font_color_suffix} 使用 BBR 加速
 ${Green_font_prefix}4.${Font_color_suffix} 使用 BBR 魔改版加速
 ${Green_font_prefix}5.${Font_color_suffix} 使用暴力 BBR 魔改版加速（不支持部分系统）
 ${Green_font_prefix}6.${Font_color_suffix} 使用 Lotserver（锐速）加速
————————————杂项管理————————————
